...
// define handlers needed for setup mode
...
// 1. Common Props: Always required regardless of mode
...
// 2. Setup Mode Props: Required only when creating/joining specifically
interface SetupModeProps extends CommonProps {
    mode: 'setup'; // The "discriminant"
    handlers: SetupHandlers;
    setError: (error: string | null) => void;
}

// 3. Selection Mode Props: Required only when choosing an existing org
interface SelectionModeProps extends CommonProps {
    mode: 'selection'; // The "discriminant"
    orgList: Organization[];
    selectedOrgId: string;
    setSelectedOrgId: (id: string) => void;
    handleOrgSelectSubmit: (e: FormEvent) => void;
    // Note: orgIdInput/setOrgIdInput removed as they aren't used in selection UI
}

// The Discriminated Union: Props can be EITHER setup mode OR selection mode
export type StaffOrgConnectProps = SetupModeProps | SelectionModeProps;


const StaffOrgConnect: React.FC<StaffOrgConnectProps> = (props) => {
    // Destructure common props that are always available
    const { userId, error, isAuthenticating } = props;

    // --- State for Initial Setup Mode (Internal UI state) ---
    // These hooks must run unconditionally at the top level.
    ...
    // These handlers use type guards to ensure they only run when props are valid.

    const handleCreateSubmit = (e: FormEvent) => {
        e.preventDefault();

        // Type Guard: We can only submit creation if we are in setup mode.
        if (props.mode !== 'setup') {
            console.error("Cannot handle create submit while in selection mode");
            return;
        }

        props.setError(null);
        if (orgName.trim() && roleChoice) {
            // TypeScript knows props.handlers exists here because of the guard above
            props.handlers.handleOrgConnect(orgName.trim(), roleChoice, null);
        } else {
            props.setError("Please enter an organization name and select a role.");
        }
    };
    ...
    const handleJoinSubmit = (e: FormEvent) => {
        ...
        // Type Guard
        if (props.mode !== 'setup') {
            console.error("Cannot handle join submit while in selection mode");
            return;
        }

        props.setError(null);
        if (orgIdToJoin.trim()) {
            props.handlers.handleOrgConnect(null, 'Staff', orgIdToJoin.trim());
        } else {
            props.setError("Please enter a valid Organization ID to join.");
        }
    };
    ...
    const renderCommonHeader = () => (
        <>
            ...
            <p className="text-sm text-gray-500 text-center">
                User ID: <code className="text-indigo-600 font-mono text-xs">{userId}</code>
            </p>
            {error && (
    ...
    // RENDER: Organization Selection UI
    const renderSelectionMode = () => {
        // Type Guard: Ensure we have selection props
        if (props.mode !== 'selection') return null;

        // Destructure selection-specific props now that TS knows they exist
        const { orgList, selectedOrgId, setSelectedOrgId, handleOrgSelectSubmit } = props;
    ...
    // RENDER: Initial Setup UI (Create or Join)
    const renderInitialSetupMode = () => {
        // Type Guard: Although we don't use specific props here other than isAuthenticating (which is common),
        // this ensures this block only renders in the correct mode.
            ...
            <>
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl shadow-inner">
                    <button
                        ...
                        className={`flex-1 p-3 rounded-xl font-semibold transition-colors ${setupTab === 'create' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 p-3 rounded-xl font-semibold transition-colors ${setupTab === 'join' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200'}`}
                    >
    ...
    return (
        <div className="max-w-md w-full mx-auto p-8 bg-white rounded-2xl shadow-2xl space-y-6">
            {renderCommonHeader()}
            {/* Conditional rendering based on the discriminator 'mode' */}
            {props.mode === 'selection' ? renderSelectionMode() : renderInitialSetupMode()}
        </div>
    );
};
