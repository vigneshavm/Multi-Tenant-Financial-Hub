...
import { getPurchaseDisplayStatus } from '../../utils/dataUtils'; 

const PurchaseHistory = ({ handlers, state, calculations }) => {
    
    // Using a function defined outside the main component scope for access (imported from dataUtils)
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">Purchase History & Bill Due Monitoring</h2>
            <p className="text-gray-500 mb-4">Viewing purchases made between {calculations.dateRange.start.toLocaleDateString()} and {calculations.dateRange.end.toLocaleDateString()}.</p>
            ...
            <ul className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                {calculations.filteredPurchasesByStatus.length === 0 ? (
                    ...
                    calculations.filteredPurchasesByStatus.map(item => {
                        ...
                        return (
                            <li key={item.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 transition-shadow hover:shadow-md">
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-gray-900">{item.vendor} - Purchase: ₹{item.amount.toLocaleString('en-IN')}</p>
                                    <div className="mt-1 flex flex-wrap items-center gap-2">
                                        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${statusColor}`}>{statusText}</span>
                                        <span className="text-xs text-gray-700">Purchased: {new Date(item.dateLogged).toLocaleDateString('en-IN')}</span>
                                        <span className="text-xs text-red-500 font-semibold">Due: {new Date(item.expectedPaymentDate).toLocaleDateString('en-IN')}</span>
                                    </div>
