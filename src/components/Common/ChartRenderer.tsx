...
const ChartRenderer: React.FC<ChartRendererProps> = ({ data, labels, colors, title }) => {
  // 2. Type the ref as an HTMLCanvasElement (initialized as null)
  const chartRef = useRef<HTMLCanvasElement | null>(null);

  const renderChart = useCallback(() => {
    const canvas = chartRef.current;
    
    // 3. Guard clause: Ensure canvas exists and data is not empty
    if (!canvas || data.length === 0) return;

    // 4. Get the context and ensure it exists
    ...
    // 5. Guard clause: Ensure the parent element exists before accessing clientWidth
    if (!container) return; 
    ...
    data.forEach((value, i) => {
      ...
      // Logic: If value > 0, bar grows up from zeroY. 
      // Current logic implies standard bar chart behavior anchored at bottom.
      const y = zeroY - (value > 0 ? barHeight : 0);
      ...
      if (barHeight > 20) {
        ctx.textAlign = 'center';
        // Note: Formatting numbers explicitly as strings for fillText
        ctx.fillText(
