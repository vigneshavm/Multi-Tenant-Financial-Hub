...
import { StaffOrgConnect, PinScreen } from './components/Auth';
...
const App = () => {
    ...
    const { sales, expenses, purchases, cheques, bankBalance, orgList, currentOrgName, setCurrentOrgName } = 
        useDataFetching(isAuthReady, organizationId, role);

    // 3. Calculation Hook
    const calculations = useDataCalculations(sales, expenses, purchases);
    ...
    const handleSaveOrgName = async () => {
        ...
        } catch (e) { console.error("Error saving organization name:", e); setError("Failed to save organization name."); }
    };
    
    // 4. Data Handler Provider
    const handlers = useDataHandlers({ organizationId, role, setError });

    // Bundle state for cleaner props
    const state = { role, setError, organizationId, currentOrgName, userId };
    ...
    if (showPinScreen || !isAuthReady) {
        return (
            <PinScreen pin={pin} setPin={setPin} handlePinSubmit={() => handlePinSubmit(setActiveTab)} error={error} />
        );
    }
...
// This export structure is simplified for this environment.
export default App;
